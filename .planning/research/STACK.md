...
**Confidence:** MEDIUM (web-search cross-checked across multiple independent sources; no official-docs MCP provider was available this run, so treat exact point-release numbers as approximate and re-verify against PyPI at implementation time)
...
```python
SIM_HZ = 2  # tune 1-4Hz per PROJECT.md
FIXED_DT = 1.0 / SIM_HZ

accumulator = 0.0
previous_state = capture_state(aircraft_list)

while running:
    frame_time = clock.tick(60) / 1000.0
    frame_time = min(frame_time, 0.25)  # clamp to avoid spiral of death
    accumulator += frame_time

    while accumulator >= FIXED_DT:
        previous_state = capture_state(aircraft_list)
        sim_step(aircraft_list, FIXED_DT)   # 1-4Hz aircraft state update
        accumulator -= FIXED_DT

    alpha = accumulator / FIXED_DT
    render_state = interpolate(previous_state, capture_state(aircraft_list), alpha)
    render(render_state)  # 60fps radar canvas draw
```

This is the single most important architectural decision this research surfaced: because the sim tick (1-4Hz) is far slower than the render rate (60fps), **interpolating aircraft position/heading between the last two sim states is required**, not optional — without it, aircraft will visibly "jump" every 250ms-1s at a 1-4Hz tick rate instead of appearing to fly smoothly. Build `capture_state`/`interpolate` as first-class concepts from the start rather than retrofitting them once the radar canvas already assumes "current state = ground truth."
...
Web search (no live PyPI/Context7 lookup was available this session) gave consistent but not authoritative version numbers. Before pinning `requirements.txt`/`pyproject.toml`, re-check exact current versions directly against PyPI for: `pygame-ce`, `pygame_gui`, `pydantic`, `geographiclib`. The architectural recommendations (pygame-ce over pygame, geographiclib over pyproj, pygame_gui over imgui-bundle, single-threaded accumulator loop over asyncio/threading) are independent of exact point-release numbers and are HIGH-confidence directional calls even though the specific version strings are MEDIUM-confidence.
...
---
*Stack research for: single-airport ATC simulator (Python + Pygame desktop app)*
*Researched: 2026-07-03*
